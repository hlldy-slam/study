...
#pragma omp parallel
    {
        ...
        for (int loc_index = 0; loc_index < (int)vec_sweepLocation.size(); ++loc_index) {
            ...
            double forward_hit_extend_distance = 0.5; // TODO: 可改为配置项 (米)
            ...
            if (forward_hit_extend_distance > 0.0 && step_len > 0.0) {
                ...
                using CloudElemT = std::remove_reference<decltype(cloud[0])>::type;
                std::vector<CloudElemT> extended_hits;
                ...
                for (auto const& eh : extended_hits) {
                    cloud.push_back(eh);
                }
            }
            // ======================================================
