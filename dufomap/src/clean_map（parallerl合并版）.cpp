        ...
        for (int loc_index = 0; loc_index < (int)vec_sweepLocation.size(); ++loc_index) {
            ...
            // use camera traj + flann (localFlann) to mark near-camera points -> mark remove vs keep
            {
                ...
                for (size_t pi = 0; pi < pc_nonground->points_.size(); ++pi) {
                    ...
                    // tmp_pt[0].x = r(0,0)*x + r(0,1)*y + r(0,2)*z + t.x;
                    // tmp_pt[0].y = r(1,0)*x + r(1,1)*y + r(1,2)*z + t.y;
                    // tmp_pt[0].z = r(2,0)*x + r(2,1)*y + r(2,2)*z + t.z;
                ...
                if (pointsB.rows > 0) {
                    ...
                    for (size_t j = 0; j < vec_label.size(); ++j) {
                        if (vec_label[j] == 1) {
                            ...
                            // tmp_pt[0].x = r(0,0)*p.x() + r(0,1)*p.y() + r(0,2)*p.z() + t.x;
                            // tmp_pt[0].y = r(1,0)*p.x() + r(1,1)*p.y() + r(1,2)*p.z() + t.y;
                            // tmp_pt[0].z = r(2,0)*p.x() + r(2,1)*p.y() + r(2,2)*p.z() + t.z;
                            // if (!pc_nonground->colors_.empty()) {
                            //     auto col = pc_nonground->colors_[j];
                            //     tmp_pt[0].red = static_cast<unsigned char>(std::clamp(int(std::round(col.x()*255.0)),0,255));
                            //     tmp_pt[0].green = static_cast<unsigned char>(std::clamp(int(std::round(col.y()*255.0)),0,255));
                            //     tmp_pt[0].blue = static_cast<unsigned char>(std::clamp(int(std::round(col.z()*255.0)),0,255));
                            // } else tmp_pt[0].red = tmp_pt[0].green = tmp_pt[0].blue = 255;
                            // local_cloud_remove.push_back(tmp_pt[0]);
                        } else {
            ...
            // 非地面点 -> 判断 map.seenFree，累积到 local_static / local_remove
            for (size_t idx2 = 0; idx2 < pc_nonground->points_.size(); ++idx2) {
                const auto &pp = pc_nonground->points_[idx2];
                double x = pp.x();
                double y = pp.y();
                double z = pp.z();
                ...
                // ufo::PointCloudColor tmp_pt(1);
                // tmp_pt[0].x = r(0,0)*pp.x() + r(0,1)*pp.y() + r(0,2)*pp.z() + t.x;
                // tmp_pt[0].y = r(1,0)*pp.x() + r(1,1)*pp.y() + r(1,2)*pp.z() + t.y;
                // tmp_pt[0].z = r(2,0)*pp.x() + r(2,1)*pp.y() + r(2,2)*pp.z() + t.z;
                if (!pc_nonground->colors_.empty()) {
                    ...
                    //tmp_pt[0].red = tmp_pt[0].green = tmp_pt[0].blue = 255;
                    pt[0].red = pt[0].green = pt[0].blue = 255;
                }

                if (!map.seenFree(pt[0])) {
                    //local_cloud_static.push_back(tmp_pt[0]);
                    local_cloud_static.push_back(pt[0]);
                } else {
                    pt[0].red = 255; pt[0].green = 0; pt[0].blue = 0;
                    local_cloud_remove.push_back(pt[0]);
                }
            }
