...
#pragma omp parallel
    {
        ...
        for (int loc_index = 0; loc_index < (int)vec_sweepLocation.size(); ++loc_index) {
            ...
            // 进度输出：避免使用可能非线程安全的 timing 对象（lastSeconds），改为简单比例显示
            if (cur > 1 && !m_config.printing.verbose) {
                std::ostringstream log_msg;
                log_msg << "(" << cur << "/" << num_location << ") Processing: " << ele.uuid;
                std::string spaces(6, ' ');
                log_msg << spaces;
                {
            ...
            {
                // 帧统计稳定度：使用原始点的世界坐标（扩展点不参与稳定度）
                if (cloud_raw.size() > cloud.size()) {
                    // 理论不应发生，防御性检查
                    std::lock_guard<std::mutex> lock(cout_mutex);
                    std::cerr << "[WARN] cloud_raw.size() > cloud.size() (" << cloud_raw.size() << "," << cloud.size() << ") skipped frame stats\n";
                } else if (!cloud_raw.empty()) {
                    std::unordered_set<uint64_t> frame_unique_hits;
                    frame_unique_hits.reserve(std::min<size_t>(cloud_raw.size(), 4096));
                    // 使用 cloud[i_pt]（已变换）对应原始索引范围
            ...
            for(size_t ip = 0; ip < pc_ground->points_.size(); ++ip){
                ...
                //以下这段注释了会报错
                // 法线未必存在，若长度不可靠则置零，避免未初始化值传播
                // Eigen::Vector3d normal = (pc_ground->normals_.empty() || ip >= pc_ground->normals_.size()) ? Eigen::Vector3d(0,0,0) : pc_ground->normals_[ip];
