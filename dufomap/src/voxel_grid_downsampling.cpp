...
std::vector<Point> VoxelGridDownSampler::downsample(const std::vector<Point> &input_cloud, bool apply_averaging)
{
    ...
    if (apply_averaging)
    {
        ...
        {
            VoxelKey key{static_cast<std::int64_t>(std::floor(point.x * inv_leaf_size_x_)),
                         static_cast<std::int64_t>(std::floor(point.y * inv_leaf_size_y_)),
                         static_cast<std::int64_t>(std::floor(point.z * inv_leaf_size_z_))};
