...
// ================== 动态稳定度辅助结构 ==================
struct VoxelFrameStat {
    int first_frame = -1;   // 首次出现帧
    int last_frame = -1;    // 最后出现帧
    int hit_frames = 0;     // 不同帧中被命中次数 (同帧多次命中只计 1)
};

// 用分辨率量化坐标到体素键，组合成 64-bit key
inline uint64_t voxelKey(double x, double y, double z, double res) {
    // 使用 21 bits per axis (signed) 打包：[-1M,1M] 以内足够，一般分辨率下 ok
    ...
    // 转换为无符号并截断到 21 bits
    const uint64_t bix = (static_cast<uint64_t>(ix) & 0x1FFFFFULL); // 21 bits
    const uint64_t biy = (static_cast<uint64_t>(iy) & 0x1FFFFFULL);
    const uint64_t biz = (static_cast<uint64_t>(iz) & 0x1FFFFFULL);
    return (bix << 42) | (biy << 21) | biz; // [ix|iy|iz]
}
                               ...
                               int nb_neighbors, double std_ratio)
{
    ...
    for (long i = 0; i < (long)n; ++i) {
        ...
        for (int k = 0; k < found; ++k) {
            if (indices_tmp[k] == (int)i) continue;
...
void CleanMap::run(){
    ...
    omp_set_num_threads(m_config.integration.num_threads);
    ...
    // 全局输出容器：
    // cloud_static_all    -> 最终优化后的静态点
    // cloud_static        -> 静态点（地面 + 非地面离线分类得到）
    // cloud_remove        -> 近相机过滤 + 动态(voxels seenFree) + 其他规则
    // cloud_acc_ground    -> 汇总地面点（保留），也会被并入 cloud_static
    // cloud_acc_noise     -> 噪声点（若后续使用）
    // candidate_non_ground-> 第一阶段收集的“待离线分类”的非地面、非近摄像机移除点（世界坐标）
    ufo::PointCloudColor cloud_static_all;
    ...
    ufo::PointCloudColor candidate_non_ground; // 离线 seenFree 分类候选
    ufo::PointCloudColor candidate_non_ground_height; // 离线高楼部分候选

    // 全局体素帧统计：key -> VoxelFrameStat
    std::unordered_map<uint64_t, VoxelFrameStat> voxel_frame_stats;
    voxel_frame_stats.reserve(1000000); // 预留一些，避免频繁 rehash
    double res = m_config.map.resolution;
...
#pragma omp parallel
    {
        ...
        // 每线程本地累积容器，减少锁竞争
        ufo::PointCloudColor local_cloud_static;         // 仅用于地面点静态直接加入（不含待分类的非地面）
        ufo::PointCloudColor local_cloud_remove;         // 近摄像机剔除的点
        ufo::PointCloudColor local_cloud_acc_ground;     // 地面汇总（与 local_cloud_static 重复角色可合并，这里保持清晰）
        ufo::PointCloudColor local_cloud_acc_noise;
        ufo::PointCloudColor local_candidate_non_ground; // 第一阶段收集的非地面候选（尚未 seenFree 判定）
        ufo::PointCloudColor local_candidate_non_ground_height; // 第一阶段收集的非地面候选中的高楼部分，粗略过滤动态物即可
        ...
        for (int loc_index = 0; loc_index < (int)vec_sweepLocation.size(); ++loc_index) {
            ...
            // ================= 前向 hit 扩展 (ds) =================
            // 论文中描述: 在实际传感器噪声下, 命中点前方 ds 距离内的体素也视为 hit, 以避免
            // 误差导致无法在障碍附近形成闭包。此处简易实现: 在每个原始命中点沿射线方向
            // 前向追加若干“虚拟命中”点。
            // 可调参数: forward_hit_extend_distance 与 forward_hit_extend_step (步长)。
            // 目前没有在 IntegrationParams 中定义, 这里先使用常量或从配置中读取占位。
            double forward_hit_extend_distance = m_config.map.forward_hit_extend_distance; // TODO: 可改为配置项 (米)
            double step_len = m_config.map.resolution; // 使用地图分辨率作为步长
            if (forward_hit_extend_distance > 0.0 && step_len > 0.0) {
                const size_t original_size = cloud.size();
                // 每点最大扩展数量按当前步长计算
                const std::size_t max_extra_per_point = static_cast<std::size_t>(std::ceil(forward_hit_extend_distance / step_len));
                // 单独缓存扩展点，避免在原向量上并发扩展导致潜在缓存行写放大
                // 使用与 cloud 相同的容器类型，便于直接 insert 合并
                ufo::PointCloudColor extended_hits;
                extended_hits.reserve(original_size * max_extra_per_point);
                ufo::Point sensor_origin = viewpoint.translation;
                for (size_t i_pt = 0; i_pt < original_size; ++i_pt) {
                    const auto base_pt = cloud[i_pt];
                    double dx = base_pt.x - sensor_origin.x;
                    double dy = base_pt.y - sensor_origin.y;
                    double dz = base_pt.z - sensor_origin.z;
                    double norm = std::sqrt(dx*dx + dy*dy + dz*dz);
                    if (norm <= 1e-6) continue; // 跳过原点
                    dx /= norm; dy /= norm; dz /= norm;
                    for (std::size_t s = 1; s <= max_extra_per_point; ++s) {
                        double dist = s * step_len;
                        if (dist > forward_hit_extend_distance) break;
                        auto extra = base_pt; // 复制颜色
                        extra.x = base_pt.x + dx * dist;
                        extra.y = base_pt.y + dy * dist;
                        extra.z = base_pt.z + dz * dist;
                        extended_hits.push_back(extra);
                    }
                }
                if (!extended_hits.empty()) {
                    std::lock_guard<std::mutex> lock(cout_mutex);
                    cloud_raw.insert(cloud_raw.end(), extended_hits.begin(), extended_hits.end());
                }
            }
            // ======================================================
            ...
            {
                std::lock_guard<std::mutex> guard(mtx);
                ufo::insertPointCloud(map, cloud_raw, viewpoint.translation, m_config.integration, m_config.propagate);
            }

            // ============ 帧统计稳定度收集 ============
            // 本帧去重命中体素（只统计原始命中点，不含扩展点 extended_hits）
            // original_size 是扩展前大小
            {
                std::unordered_set<uint64_t> frame_unique_hits;
                frame_unique_hits.reserve(1024);
                for (size_t i_pt = 0; i_pt < cloud.size(); ++i_pt) {
                    // 因为 map 基于世界坐标索引
                    auto const& wpt = cloud[i_pt];
                    uint64_t key = voxelKey(wpt.x, wpt.y, wpt.z, res);
                    frame_unique_hits.insert(key);
                }
                // 合并到全局统计（加锁）
                if (!frame_unique_hits.empty()) {
                    std::lock_guard<std::mutex> guard(mtx);
                    for (auto key : frame_unique_hits) {
                        auto &st = voxel_frame_stats[key];
                        if (st.first_frame == -1) st.first_frame = loc_index;
                        st.last_frame = loc_index;
                        st.hit_frames += 1; // 本帧出现一次
                    }
                }
            }
            // =========================================
            ...
            auto cloud_cut_nonground_height = std::make_shared<open3d::geometry::PointCloud>();
            ...
            if (bfilterGroundPlane) {
                ...
                for (size_t ii = 0; ii < vec_points.size(); ++ii) {
                    ...
                    double y_min = m_config.groundSeg.ground_min_y;
                    double y_max = m_config.groundSeg.ground_max_y;
                    double y_max_height = m_config.groundSeg.ground_max_y_height;
                    ...
                    } else if(p3.y() <= y_max_height){
                        cloud_cut_nonground_height->points_.push_back(p3);
                        cloud_cut_nonground_height->colors_.push_back(c3);
                        cloud_cut_nonground_height->normals_.push_back(n3);
                    } else {
            ...
            for(size_t ip = 0; ip < pc_ground->points_.size(); ++ip){
                ...
                // 现在 pc_ground 已是世界坐标点，不再进行二次位姿变换
                ufo::PointCloudColor pt_ground_filter(1);
                pt_ground_filter[0].x = p.x();
                pt_ground_filter[0].y = p.y();
                pt_ground_filter[0].z = p.z();
                pt_ground_filter[0].red   = 255;
                pt_ground_filter[0].green = 255;
                pt_ground_filter[0].blue  = 0;
                // ufo::applyTransform(pt_ground_filter, viewpoint);
                // 多尺度 seenFree 判断：只要所有选定深度都未 seenFree 则视为静态
                // auto isStaticMultiScale = [&](ufo::Point const& wp) {
                //     // 深度层级选择策略：从细到粗，0 表示默认最细；可根据分辨率层级动态生成
                //     // 假设 m_config.map.levels 给出最大层级数，选取前若干层做多尺度一致性判断
                //     std::array<int,5> depths{0, 1, 3, 5, 10}; // 可调整为其他层级
                //     int max_levels = static_cast<int>(m_config.map.levels);
                //     for (int d : depths) {
                //         if (d >= max_levels) break; // 超出实际层级则停止
                //         if (map.seenFree(wp, d)) {
                //             return false; // 任一尺度已 seenFree -> 非静态
                //         }
                //     }
                //     return true;
                // };
                // 判断静态：此处暂保持原逻辑 (always true)，避免地面点重复变换导致偏移
                if (1) {
                    ...
                    std::cout<<"ground point seenFree false error!"<<std::endl;
                    pt_ground_filter[0].green = 0;
                    local_cloud_remove.push_back(pt_ground_filter[0]);
                }
            ...
            // use camera traj + flann (localFlann) to mark near-camera points -> mark remove vs keep
            {
                ...
                for (size_t pi = 0; pi < pc_nonground->points_.size(); ++pi) {
                    ...
                    if (y > -0.2 && y < 0.2) {
                        ...
                        pointsB.at<float>(count, 0) = static_cast<float>(x);
                        pointsB.at<float>(count, 1) = static_cast<float>(y);
                        pointsB.at<float>(count, 2) = static_cast<float>(z);
                        ++count;
                    }
                }
            ...
            TN_vec = N_vec;
            // 非地面点（向量化世界坐标 TN_vec） -> 第一阶段仅缓存候选，第二阶段统一离线分类
            // 这样可使用全量地图（所有 sweeps 插入后的状态）执行 seenFree 判定，避免“同帧插入后立即判定”带来的偏差。
            for (size_t idx2 = 0; idx2 < nongroundN_vec; ++idx2) {
                ...
                local_candidate_non_ground.push_back(tmp_elem[0]);
            }

            for(size_t i = 0; i < cloud_cut_nonground_height->points_.size(); ++i){
                const auto &p = cloud_cut_nonground_height->points_[i];
                ufo::PointCloudColor pt_nonground_height(1);
                pt_nonground_height[0].x = p.x();
                pt_nonground_height[0].y = p.y();
                pt_nonground_height[0].z = p.z();
                pt_nonground_height[0].red   = static_cast<unsigned char>(cloud_cut_nonground_height->colors_[i].x() * 255.0);
                pt_nonground_height[0].green = static_cast<unsigned char>(cloud_cut_nonground_height->colors_[i].y() * 255.0);
                pt_nonground_height[0].blue  = static_cast<unsigned char>(cloud_cut_nonground_height->colors_[i].z() * 255.0);

                local_candidate_non_ground_height.push_back(pt_nonground_height[0]);
            }
        ...
        // 合并线程本地累积到全局累积（加锁）
        {
            ...
            if (!local_candidate_non_ground.empty())
                candidate_non_ground.insert(candidate_non_ground.end(), local_candidate_non_ground.cbegin(), local_candidate_non_ground.cend());
            if (!local_candidate_non_ground_height.empty())
                candidate_non_ground_height.insert(candidate_non_ground_height.end(), local_candidate_non_ground_height.cbegin(), local_candidate_non_ground_height.cend());
        }
    } // end parallel
    ...
    std::cout << std::endl << "build + preprocess cost: " << seconds_map << "s" << std::endl;

    // 第一阶段结束：所有 sweep 点云已插入 map，所有非地面候选已收集。
    // 可选：若 m_config.propagate == false，可以在离线分类前进行一次传播，确保层级/占据一致性。
    if (!m_config.propagate) {
        map.propagateModified();
    }

    // ================= VOID 体素集合构建 =================
    // 原始实现要求邻域全部已知(all_known)过于严格, 导致 void_voxels 数量偏少, 易漏检动态物。
    // 改为: 统计邻域已知比例 known_ratio = known_count / total_neighbors, 若 >= void_known_ratio_min 则认为是 void。
    // void_known_ratio_min 可调 (默认 0.8)。dp (邻域半径) 默认 1。
    int dp = (m_config.map.dp > 0 ? m_config.map.dp : 1);
    double void_known_ratio_min = m_config.map.void_known_ratio_min;
    std::unordered_set<uint64_t> void_voxels;
    void_voxels.reserve(200000);
    double resolution = m_config.map.resolution;

    // 预计算邻域偏移
    std::vector<std::array<int,3>> neighbor_offsets;
    neighbor_offsets.reserve((2*dp+1)*(2*dp+1)*(2*dp+1)-1);
    for (int dx=-dp; dx<=dp; ++dx) {
        for (int dy=-dp; dy<=dp; ++dy) {
            for (int dz=-dp; dz<=dp; ++dz) {
    ...
    // 遍历地图叶节点（只读）: 这里假设 UFOMap 支持 iteration via predicate query
    // 近似 intersected 候选: free 且非 occupied
    {
        std::vector<ufo::Point> free_leaf_points;
        free_leaf_points.reserve(100000);
        // 使用 UFOMap 现有查询谓词，选择 free 叶子节点: Leaf(depth=0) && SeenFree && !Occupied
        // 如果 API 不支持直接枚举，这里需要后续替换为库提供的迭代方式。暂做保护性 try 结构伪代码。
        try {
            for (auto node : map.query(ufo::pred::Leaf(0) && ufo::pred::SeenFree())) {
                // node 为值类型, 直接使用 map.center(node) 获取中心坐标
                auto c = map.center(node);
                free_leaf_points.emplace_back(c.x, c.y, c.z);
            }
        } catch (...) {
            // 如果 query 不可用，可在后续版本加入手动遍历
        }

        // 闭包检查
        #pragma omp parallel
        {
            std::vector<uint64_t> local_void;
            local_void.reserve(4096);
            #pragma omp for schedule(static)
            for (std::size_t i=0; i<free_leaf_points.size(); ++i) {
                const auto &ptc = free_leaf_points[i];
                int known_count = 0;
                int total_n = static_cast<int>(neighbor_offsets.size());
                for (auto &off : neighbor_offsets) {
                    double nx = ptc.x + off[0]*resolution;
                    double ny = ptc.y + off[1]*resolution;
                    double nz = ptc.z + off[2]*resolution;
                    ufo::Point np(nx, ny, nz);
                    // 近似 known 判定: seenFree 任一尺度即可; 可扩展更多深度
                    bool known = map.seenFree(np);
                    if (known) ++known_count;
                }
                double ratio = total_n > 0 ? static_cast<double>(known_count) / total_n : 0.0;
                if (ratio >= void_known_ratio_min) {
                    local_void.push_back(voxelKey(ptc.x, ptc.y, ptc.z, resolution));
                }
            }
            #pragma omp critical
            {
                void_voxels.insert(local_void.begin(), local_void.end());
            }
        }
    }

    std::cout << "void voxels computed: " << void_voxels.size() << std::endl;

    // 离线第二阶段：使用 void voxel 集合判定 candidate_non_ground / height
    auto classify_start = high_resolution_clock::now();
    {
        // ufo::PointCloudColor classified_static;  // 新增的非地面静态
        // ufo::PointCloudColor classified_remove;  // 新增的非地面动态

        if (!cloud_static.empty())
            cloud_static_all.insert(cloud_static_all.end(), cloud_static.cbegin(), cloud_static.cend());

        int kMinHitFrames = m_config.map.kMinHitFrames;          // 至少出现于多少不同帧才认为可能静态
        int kMinLifeSpanFrames = m_config.map.kMinLifeSpanFrames; // 生命周期至少多少帧才认为可能静态

        // 采用并行加速（只读 map，线程安全假设：seenFree 没有写操作）
        // #pragma omp parallel
        // {
        //     ufo::PointCloudColor local_static1;
        //     ufo::PointCloudColor local_remove1;
        //     //#pragma omp for schedule(dynamic, 4096)
        //     #pragma omp for schedule(static)
        //     for (size_t i = 0; i < cloud_static.size(); ++i) {
        //         auto p = cloud_static[i];
        //         bool keep = true;
        //         {
        //             std::array<int,5> depths{0, 1, 3, 5, 10};
        //             int max_levels = static_cast<int>(m_config.map.levels);
        //             for (int d : depths) {
        //                 if (d >= max_levels) break;
        //                 if (map.seenFree(p, d)) { keep = false; break; }
        //             }
        //         }
        //         if (keep) {
        //             // 帧稳定度再检查
        //             uint64_t key = voxelKey(p.x, p.y, p.z, res);
        //             auto it = voxel_frame_stats.find(key);
        //             if (it != voxel_frame_stats.end()) {
        //                 const auto &st = it->second;
        //                 int life = st.last_frame - st.first_frame + 1;
        //                 if (!(st.hit_frames >= kMinHitFrames && life >= kMinLifeSpanFrames)) {
        //                     keep = false; // 不满足稳定度，转动态
        //                 }
        //             } else {
        //                 keep = false; // 没统计信息默认不稳定
        //             }
        //         }
        //         if (keep) {
        //             local_static1.push_back(p);
        //         } else {
        //             p.red = 255; p.green = 0; p.blue = 0;
        //             local_remove1.push_back(p);
        //         }
        //     }
        //     #pragma omp critical
        //     {
        //         if (!local_static1.empty())
        //             cloud_static_all.insert(cloud_static_all.end(), local_static1.cbegin(), local_static1.cend());
        //         if (!local_remove1.empty())
        //             cloud_remove.insert(cloud_remove.end(), local_remove1.cbegin(), local_remove1.cend());
        //     }
        // }

        // 高楼部分: 改为统计滤波 (Open3D 统计离群点移除)
        // 将 candidate_non_ground_height 构造成 Open3D 点云并执行 RemoveStatisticalOutlierOpen3D
        // 使用与 filterNoise 相同的参数 (MeanK / StddevMulThresh)，用户可在配置里调整

        // if (!candidate_non_ground_height.empty()) {
        //     auto pc_height = std::make_shared<open3d::geometry::PointCloud>();
        //     // 预分配并用索引写入，便于并行化
        //     pc_height->points_.resize(candidate_non_ground_height.size());
        //     pc_height->colors_.resize(candidate_non_ground_height.size());
        //     #pragma omp parallel for schedule(static)
        //     for (std::size_t i = 0; i < candidate_non_ground_height.size(); ++i) {
        //         const auto &p = candidate_non_ground_height[i];
        //         pc_height->points_[i] = Eigen::Vector3d(p.x, p.y, p.z);
        //         pc_height->colors_[i] = Eigen::Vector3d(p.red/255.0, p.green/255.0, p.blue/255.0);
        //     }

        //     // 若点数过少，不做统计滤波，直接全部保留
        //     std::vector<char> is_inlier_height(candidate_non_ground_height.size(), 1);
        //     if (pc_height->points_.size() >= 5) { // 简单阈值，避免 mean_k>size 情况
        //         int mean_k = std::max(1, m_config.filterNoise.filterMeanK);
        //         double std_ratio = m_config.filterNoise.StddevMulThresh;
        //         std::shared_ptr<open3d::geometry::PointCloud> inlier_pc_h;
        //         std::vector<size_t> inlier_indices_h;

        //         std::cout << "filtering, waiting..." << std::endl;
        //         std::tie(inlier_pc_h, inlier_indices_h) = RemoveStatisticalOutlierOpen3D(pc_height, mean_k, std_ratio);

        //         // 标记 inlier
        //         std::fill(is_inlier_height.begin(), is_inlier_height.end(), 0);
        //         // 标记 inlier: 使用并行写入，inlier_indices_h 元素互不冲突
        //         #pragma omp parallel for schedule(static)
        //         for (std::size_t k = 0; k < inlier_indices_h.size(); ++k) {
        //             auto idx = inlier_indices_h[k];
        //             if (idx < is_inlier_height.size()) {
        //                 is_inlier_height[idx] = 1;
        //             }
        //         }
        //     }

        //     // 并行分类: inlier -> static_all, outlier -> remove (染红)
        //     #pragma omp parallel
        //     {
        //         ufo::PointCloudColor local_static1;
        //         ufo::PointCloudColor local_remove1;
        //         #pragma omp for schedule(static)
        //         for (size_t i = 0; i < candidate_non_ground_height.size(); ++i) {
        //             auto p = candidate_non_ground_height[i];
        //             bool keep = is_inlier_height[i];
        //             if (keep) {
        //                 local_static1.push_back(p);
        //             } else {
        //                 p.red = 255; p.green = 0; p.blue = 0; // 标记为移除点
        //                 local_remove1.push_back(p);
        //             }
        //         }
        //         #pragma omp critical
        ...
        //     long long duration_filter = duration_cast<milliseconds>(end_filter - begin_t).count();
        //     float seconds_filter = duration_filter / 1'000.0f;
        //     std::cout << "filter cost time: " << seconds_filter << "s" << std::endl;
        // }
        ...
        // 使用 void voxel 替代 seenFree + 稳定度双重确认：只有在 void 中并且稳定度不足才判为动态
        #pragma omp parallel
        {
            ufo::PointCloudColor local_static2;
            ufo::PointCloudColor local_remove2;
            #pragma omp for schedule(static)
            for (size_t i = 0; i < candidate_non_ground.size(); ++i) {
                auto p = candidate_non_ground[i];
                uint64_t key = voxelKey(p.x, p.y, p.z, res);
                bool is_void = (void_voxels.find(key) != void_voxels.end());
                bool keep = true;
                if (is_void) {
                    // 稳定度: 若命中次数和寿命均不足则判动态，否则仍保留
                    auto it = voxel_frame_stats.find(key);
                    if (it != voxel_frame_stats.end()) {
                        const auto &st = it->second;
                        int life = st.last_frame - st.first_frame + 1;
                        ...
                        keep = false; // 无统计信息的 void -> 动态假设
                    }
                }
                if (keep) {
                    local_static2.push_back(p);
                } else {
                    p.red = 255; p.green = 0; p.blue = 0;
                    local_remove2.push_back(p);
                }
            }
            #pragma omp critical
            {
                if (!local_static2.empty())
                    cloud_static_all.insert(cloud_static_all.end(), local_static2.cbegin(), local_static2.cend());
                if (!local_remove2.empty())
                    cloud_remove.insert(cloud_remove.end(), local_remove2.cbegin(), local_remove2.cend());
            }
        }

        // ================= 原始 seenFree 策略对比 =================
        // 通过多尺度 seenFree 判定：若任一选定深度 seenFree 则视为动态，再结合稳定度；结果单独保存方便调试。
        bool enable_seenfree_debug = m_config.map.enable_seenfree_debug; // 可改成读取配置
        ufo::PointCloudColor cloud_static_seenfree;
        ufo::PointCloudColor cloud_remove_seenfree;
        if (enable_seenfree_debug) {
            std::array<int,5> depths{0,1,3,5,10};
            #pragma omp parallel
            {
                ufo::PointCloudColor local_static_sf;
                ufo::PointCloudColor local_remove_sf;
                #pragma omp for schedule(static)
                for (size_t i = 0; i < candidate_non_ground.size(); ++i) {
                    auto p = candidate_non_ground[i];
                    bool keep = true;
                    ...
                    for (int d : depths) {
                        if (d >= max_levels) break;
                        if (map.seenFree(p, d)) { // 任一尺度 free -> 动态候选
                            keep = false;
                            break;
                        }
                    }
                    ...
                    if (keep) {
                        local_static_sf.push_back(p);
                    } else {
                        auto q = p;
                        q.red = 255; q.green = 0; q.blue = 0;
                        local_remove_sf.push_back(q);
                    }
                }
                #pragma omp critical
                {
                    if (!local_static_sf.empty())
                        cloud_static_seenfree.insert(cloud_static_seenfree.end(), local_static_sf.cbegin(), local_static_sf.cend());
                    if (!local_remove_sf.empty())
                        cloud_remove_seenfree.insert(cloud_remove_seenfree.end(), local_remove_sf.cbegin(), local_remove_sf.cend());
                }
            }
            std::cout << "[DEBUG] seenFree static: " << cloud_static_seenfree.size() << " remove: " << cloud_remove_seenfree.size() << std::endl;
        }
        // 将 seenFree 结果单独输出文件（不并入 cloud_static_all），用于与 void 策略对比
        if (enable_seenfree_debug && m_config.fdageParam.output_map) {
            ufo::writePointCloudPLY_downsample(m_data_root + (m_config.output.filename + "_seenfree_static.ply"), cloud_static_seenfree, 0.1,0.1,0.1);
            ufo::Color remove_color(255,0,0);
            writePointCloudPLY_downsample_remove(m_data_root + (m_config.output.filename + "_seenfree_remove.ply"), cloud_remove_seenfree, 0.1,0.1,0.1, remove_color);
        }

        // 合并：地面静态 + 非地面静态
        // cloud_static.insert(cloud_static.end(), classified_static.cbegin(), classified_static.cend());
        // cloud_remove.insert(cloud_remove.end(), classified_remove.cbegin(), classified_remove.cend());
    }
    auto classify_end = high_resolution_clock::now();
    long long classify_ms = std::chrono::duration_cast<std::chrono::milliseconds>(classify_end - classify_start).count();

    // 把合并后的结果用于后续流程（保留原来逻辑写文件 / 聚类 / 输出）
    cloud_acc_noise = std::move(cloud_acc_noise);

    indicators::show_console_cursor(true);
    std::cout << "Finished Processing data. Start saving map... " << std::endl;

    // propagate 已在离线分类前执行（若需要），此处不再重复；保留兼容逻辑：
    if (m_config.propagate) {
        // 若在插入阶段已选择实时 propagate，这里可选再次 propagate 以确保最终一致
    ...
    if(m_config.fdageParam.output_map)
    {
        ...
        ufo::writePointCloudPLY_downsample(m_data_root +  (m_config.output.filename + save_static_name), cloud_static_all, 0.1, 0.1, 0.1);
