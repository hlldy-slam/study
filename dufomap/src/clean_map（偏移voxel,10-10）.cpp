...
inline uint64_t voxelKey(double x, double y, double z, double res) {
    // 改为“偏移编码”以保留符号，避免原先按位与导致负数与大正数冲突 (alias)。
    // 约定：体素整数索引 ix = floor(x / res) 需落在 [-2^20, 2^20-1] 内；偏移 bias = 2^20。
    // 编码：stored = ix + bias 映射到 [0, 2^21 - 1]，不再丢失符号。
    // 若越界则进行 clamp（也可选择抛异常或扩展位宽，这里采用安全截断防止崩溃）。
    // 最终 64-bit key 布局仍为 [X(21 bits)|Y(21 bits)|Z(21 bits)] = 63 bits。

    static constexpr int64_t kBias = (1LL << 20);            // 1,048,576
    static constexpr int64_t kMin  = -kBias;                  // 包含下界 (-2^20)
    static constexpr int64_t kMax  =  (1LL << 20) - 1;        // 上界 (2^20 - 1)
    static constexpr uint64_t kMask21 = (1ULL << 21) - 1ULL;  // 21 bits 掩码 (安全冗余，可不必)

    auto quant = [&](double v) -> int64_t {
        double q = std::floor(v / res);
        int64_t iv = static_cast<int64_t>(q);
        if (iv < kMin) iv = kMin; // clamp，下界被编码成 0 + bias = 0 (仍唯一)
        if (iv > kMax) iv = kMax; // clamp，上界被编码成 (2^21 - 1)
        return iv;
    };
    ...
    uint64_t bix = static_cast<uint64_t>(ix + kBias) & kMask21; // 理论上范围已保证，按位与是保险措施
    uint64_t biy = static_cast<uint64_t>(iy + kBias) & kMask21;
    uint64_t biz = static_cast<uint64_t>(iz + kBias) & kMask21;

    return (bix << 42) | (biy << 21) | biz; // [bix|biy|biz] 偏移后仍为 63 bits
}
...
void CleanMap::run(){
    ...
    voxel_frame_stats.reserve(500000); // 预留一些，避免频繁 rehash
        ...
        for (int loc_index = 0; loc_index < (int)vec_sweepLocation.size(); ++loc_index) {
            ...
            if (forward_hit_extend_distance > 0.0 && step_len > 0.0) {
                ...
                if (!extended_hits.empty()) {
                    std::lock_guard<std::mutex> lock(cout_mutex);
                    cloud.insert(cloud.end(), extended_hits.begin(), extended_hits.end());
                }
            }
            // ======================================================
            ...
            // original_size 是扩展前大小
            {
                ...
                for (size_t i_pt = 0; i_pt < cloud_raw.size(); ++i_pt) {
            ...
            for(size_t ip = 0; ip < pc_ground->points_.size(); ++ip){
                ...
                if (isStaticMultiScale(pt_ground_filter[0])) {
