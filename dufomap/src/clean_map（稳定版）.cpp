...
void CleanMap::run(){
    ...
    // 全局输出容器：
    // cloud_static        -> 最终静态点（地面 + 非地面离线分类得到）
        ...
        #pragma omp parallel
        {
            ...
            #pragma omp critical
            {
                if (!local_static2.empty())
                    classified_static.insert(classified_static.end(), local_static2.cbegin(), local_static2.cend());
                if (!local_remove2.empty())
                    classified_remove.insert(classified_remove.end(), local_remove2.cbegin(), local_remove2.cend());
            }
        }
