...
#pragma omp parallel
        ...
        for (int loc_index = 0; loc_index < (int)vec_sweepLocation.size(); ++loc_index) {
            ...
            // original_size 是扩展前大小
            {
                ...
                for (size_t i_pt = 0; i_pt < cloud.size(); ++i_pt) {
                    // 使用变换后的世界坐标统计 (cloud[i_pt]) 更合理；这里原始与世界差异：选世界坐标
                    // 因为 map 基于世界坐标索引
    ...
    // 离线第二阶段：对 candidate_non_ground 做 seenFree 分类
    auto classify_start = high_resolution_clock::now();
    {
        ...
        int kMinHitFrames = m_config.map.kMinHitFrames;          // 至少出现于多少不同帧才认为可能静态 没必要改 调大了很容易误删
