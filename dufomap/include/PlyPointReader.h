...
map<string, PlyPropertyType> plyPropertyTypes = { 
...
public:
	...
	bool readNextPoint(){
		...
		if(format == PLY_FILE_FORMAT_ASCII){
			...
			for(const auto &prop : vertexElement.properties){
				...
				else if (prop.name == "nx" && prop.type.name == plyPropertyTypes["double"].name) {
					nx = stof(token);
				}
				else if (prop.name == "ny" && prop.type.name == plyPropertyTypes["double"].name) {
					ny = stof(token);
				}
				else if (prop.name == "nz" && prop.type.name == plyPropertyTypes["double"].name) {
					nz = stof(token);
				}
			}
			...
			for(const auto &prop : vertexElement.properties){
				if(prop.name == "x" && prop.type.name == plyPropertyTypes["float"].name){
					...
					memcpy(&temp, (buffer + offset), prop.type.size);
					nx = float(temp);
				}
				else if (prop.name == "ny" && prop.type.name == plyPropertyTypes["double"].name) {
					memcpy(&temp, (buffer + offset), prop.type.size);
	...
	/*bool readNextPoint2() {
		...
		if (format == PLY_FILE_FORMAT_ASCII) {
			...
			if (tokens.size() != 9) {
				cout << "ASCII ply check failed.tokens.size() != 9. ASCII ply must include x,y,z,nx,ny,nz,r,g,b" << endl;
				return false;
			}
			x = stof(tokens[0]);
			y = stof(tokens[1]);
			...
			ny = stof(tokens[4]);
			nz = stof(tokens[5]);
			r = (unsigned char)stof(tokens[6]);
			g = (unsigned char)stof(tokens[7]);
			b = (unsigned char)stof(tokens[8]);
		}
		else if (format == PLY_FILE_FORMAT_BINARY_LITTLE_ENDIAN) {
			stream.read(buffer, pointByteSize);
	...
	AABB getAABB(){
		if(aabb == NULL){
			...
			cout << "getAABB cost time: " << (duration / 1000.0f) << "s" << endl;
		}

		return *aabb;
	}

	AABB getAABB(vector<Point>& vec_points) {
		if (aabb == NULL) {
	...
	// for ply2npy
	AABB getAABB_ply2npy(vector<Point>& vec_points) {
		if (aabb == NULL) {
			...
			PlyPointReader* reader = new PlyPointReader(file);
			/*while (reader->readNextPoint_ply2npy()) {
				Point p = reader->getPoint();
				aabb->update(p.position);
